...
function createVideo(src, loop, muted) {
  src = src.trim(); //triming src whitespaces
  if (muted === true && loop === true) {
    return `<video src="${src}" loop muted></video>`;
  } else if (loop === true) {
    return `<video src="${src}" loop></video>`;
  } else if (muted === true) {
...
//how to declare boolean datatype in javascript?
...
function fixPostalCode(postalCode) {
  ...
  postalCodes = postalCode.replace(/-/g, ' ');
  if (postalCodes[3] !== ' ') {
    postalCodes = postalCodes.substring(0, 3) + ' ' + postalCodes.substring(3, postalCodes.length);
  }

  //using a Regular expression to match pattern from the beginning/end
  var postalCode1 = /^[^DFIOQWUZ][0-9][^DFIOQU][ ][0-9][^DFIOQU][0-9]$/;
  if (postalCode1.test(postalCodes) === true) {
    return postalCodes;
  }
  throw `ERROR`; // throw an error if the given regular expression doesn't match the pattern specified
}
...
function toProvince(postalCode, useLongForm) {
  ...
  provinceRegX = /^[^DFIOQWUZ][0-9][^DFIOQU][ ]?[0-9][^DFIOQU][0-9]$/;

  //creating a set of an arrays to keep track of each provinces and it's longform
  C_province = [
    'Ontario',
    'Quebec',
    'Nova Scotia',
  ...
  longform = ['ON', 'QC', 'NS', 'NB', 'MB', 'BC', 'PE', 'SK', 'AB', 'NL', 'NT', 'YT'];
  // return null value if the specified postal code doesn't match the specified criteria..
  if (!provinceRegX.test(postalCode.toUpperCase()) === true) {
    return null;
  }

  //logic for for province and it's longForm...

  if (
    postalCode.toUpperCase()[0] === 'K' ||
    postalCode.toUpperCase()[0] === 'L' ||
    postalCode.toUpperCase()[0] === 'M' ||
    postalCode.toUpperCase()[0] === 'N' ||
    postalCode.toUpperCase()[0] === 'P'
  ) {
    province = C_province[0];
    pLongForm = longform[0];
  } else if (
    postalCode.toUpperCase()[0] === 'G' ||
    postalCode.toUpperCase()[0] === 'H' ||
    postalCode.toUpperCase()[0] === 'J'
  ) {
    province = C_province[1];
    pLongForm = longform[1];
  } else if (postalCode.toUpperCase()[0] === 'B') {
    province = C_province[2];
    pLongForm = longform[2];
  } else if (postalCode.toUpperCase()[0] === 'E') {
    province = C_province[3];
    pLongForm = longform[3];
  } else if (postalCode.toUpperCase()[0] === 'R') {
    province = C_province[4];
    pLongForm = longform[4];
  } else if (postalCode.toUpperCase()[0] === 'V') {
    province = C_province[5];
    pLongForm = longform[5];
  } else if (postalCode.toUpperCase()[0] === 'C') {
    province = C_province[6];
    pLongForm = longform[6];
  } else if (postalCode.toUpperCase()[0] === 'S') {
    province = C_province[7];
    pLongForm = longform[7];
  } else if (postalCode.toUpperCase()[0] === 'T') {
    province = C_province[8];
    pLongForm = longform[8];
  } else if (postalCode.toUpperCase()[0] === 'A') {
    province = C_province[9];
    pLongForm = longform[9];
  } else if (postalCode.toUpperCase()[0] === 'X') {
    province = C_province[10];
    pLongForm = longform[10];
  } else if (postalCode.toUpperCase()[0] === 'Y') {
    province = C_province[11];
    pLongForm = longform[11];
  } else {
    province = null;
    pLongForm = null;
  }
...
function normalizeCoord(value) {
  ...
  arrayLatLong = value.match(value1);
  ...
  // throw an errow if value is out of bound
  if (!(lati >= -90 && lati <= 90) || !(long >= -180 && long <= 180)) {
    throw 'ERROR';
  }
...
function formatCoords(...values) {
  for (var i = 0; i < values.length; i++) {
    // Checking --> errors with try and catch
    //& multiple valid lat, lng coords should be formatted correctly in a list
    try {
      if (!i) {
        var co_ordinate = normalizeCoord(values[i]);
      } else {
        co_ordinate = co_ordinate + ' ' + normalizeCoord(values[i]);
      }
    } catch (err) {
      continue;
    }
  ...
  //undefined behaviours are removed..
  if (typeof co_ordinate === 'undefined') {
    co_ordinate = '';
  }

  //list to be returned in parenthesis
  var co_odList = '(' + co_ordinate + ')';
  return co_odList;
}
...
function countForProvince(provinceCode, ...postalCodes) {
  ...
  //This validate the total number of valid postal codes
  for (var i = 0; i < postalCodes.length; i++) {
    if (provinceCode.length > 3) {
      logic = true;
    }
    if (provinceCode === toProvince(postalCodes[i], logic)) {
      totalValue++;
    }
  }
...
function generateLicenseLink(licenseCode, includeLicenseAttr) {
  ...
  if (licenseCode === 'CC-BY') {
    ...
    httplink = `"https://choosealicense.com/no-permission/"`;
  }

  if (httplink === ``) {
    charCode = licenseCode;
    charCode = charCode.substring(3, licenseCode.length).toLowerCase();
    httplink = `"https://creativecommons.org/licenses/${charCode}/4.0/"`;
  }
  if (includeLicenseAttr) {
    httplink = httplink + ` rel="license"`;
  }

  return `<a href=${httplink}>${license}</a>`;
}
...
function all() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    try {
      checkValue = toBool(arguments[i]);
    } catch (error) {
      checkValue = false;
    }
    //iterate through the checkvalue and terminate the loop if the specified checkvalue is --> false...
    if (checkValue === false) {
      return checkValue;
    }
  }
...
function some() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    try {
      success = toBool(arguments[i]);
    } catch (error) {
      success = false;
    }
    //terminate the loop if the specified success value is --> true...
    if (success === true) {
      return success;
    }
  }
...
function none() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    ...
    //terminate the loop if the value of check -> true...
    if (checkvalue === true) {
      checkvalue = false;
      return checkvalue;
    }
...
function buildQuery(query, perPage, page, format) {
  // Replace this comment with your code...
  var fileformat = ['json', 'html', 'xml'];
  var urltemplate;

  if (!fileformat.includes(format)) {
    throw 'ERROR: Enter a valid format';
  }

  //get value of page..
  if (!(page > 0)) {
    throw 'Error: Enter a valid page';
  }

  //get value of perPage
  if (!(perPage >= 1 && perPage <= 100)) {
    throw 'ERROR: Enter a valid perPage Value';
  }

  var template = `https://api.web222-example.org/v3/query?query=`;
  let url = '';
  ...
  url = `${query}&per_page=${perPage}&page=${page}&format=${format}`;
